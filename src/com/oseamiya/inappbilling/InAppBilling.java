...
public class InAppBilling extends AndroidNonvisibleComponent {
    ...
    public void PurchaseFlowLaunched() {
        EventDispatcher.dispatchEvent(this, "PurchaseFlowLaunched");
    }

    @SimpleEvent
    public void PurchaseFlowFailedToLaunch(int responseCode) {
        EventDispatcher.dispatchEvent(this, "PurchaseFlowFailedToLaunch", responseCode);
    }
    ...
    public void LaunchPurchaseFlow(Object productDetails, String offerToken, boolean isOfferPersonalized, String skuType) {
        ...
        if (billingClient.isReady()) {
            ...
            if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                ...
                PurchaseFlowFailedToLaunch(billingResult.getResponseCode());
            }
        } else {
            ...
            public void onQueryPurchasesResponse(@NonNull @NotNull BillingResult billingResult, @NonNull @NotNull List<Purchase> list) {
                activity.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                            lists.addAll(list);
                            OnQueryPurchasesResponse(YailList.makeList(lists), list.size());
                        } else {
    ...
    @SimpleEvent
    public void GotPurchaseDetails(String orderId, boolean isAcknowledged, boolean isAutoRenewing, long purchasedTime,String purchaseToken, String json) {
        EventDispatcher.dispatchEvent(this, "GotPurchaseDetails", orderId, isAcknowledged, isAutoRenewing, purchasedTime, purchaseToken, json);
    }
    ...
    public void GetPurchaseDetails(Object purchase) {
        if (purchase instanceof Purchase) {
            ...
            String purchaseToken = purchase1.getPurchaseToken();
            GotPurchaseDetails(orderId, isAcknowledge, isAutoRenewing, purchaseTime, purchaseToken, originalJson);
